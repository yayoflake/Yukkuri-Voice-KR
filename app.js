...
async function synthUnit(aq, ctx, unitKana, baseSpeed, init) {
  ...
  // 경계를 램프로 완만하게: 계단형 급변과 위상 불연속(클릭) 제거.
  // 속도는 40ms로 넉넉히(시간축 급변 방지). 피치는 12ms로 더 짧게 — 40ms면 악센트가 음절보다
  // 넓게 번져 전환이 늦게 느껴진다. 짧혀도 클릭이 안 생김은 검증함(샘플 급변이 원신호 수준).
  const tWin = Math.max(2, Math.round(sr * 0.04));
  const sWin = Math.max(2, Math.round(sr * 0.012));
  const sS = smooth(semisArr, sWin), tS = smooth(tsArr, tWin);
...
// 실시간 재생(playKana)과 빌드 스크립트(예제 음원 미리 렌더)가 같은 합성 경로를 쓰도록 분리했다.
// DOM·재생·busy 상태는 안 건드린다(순수 합성). defaultVoice/baseSpeed는 (음성)·[속도] 태그가
// 없을 때의 기본값. 반환: { full: Float32Array(끝 무음 패딩·페이드 포함), sr, silenceSecs }. 빈 입력은 throw.
...
// 버퍼 재생 (Source → Gain → 출력. 정지 시 게인 램프다운으로 클릭 방지). 고급 재생이면 하이라이트도.
...
// 공통 재생: 주어진 가나 문자열을 실시간 합성해 재생한다. btn은 UI를 표시할 버튼.
...
async function playKana(kana, btn) {
  ...
  // 음성 로드 + (x로 끊긴) 단위별 합성·후처리 후 이어붙이기 (실제 실패가 날 수 있는 구간)
  let buffer;
  try {
...
exampleList.addEventListener('click', (e) => {
  ...
  // 미리 렌더한 음원이 있으면 그걸 받아 즉시 재생(에뮬레이터 부팅 없음), 없으면 실시간 합성.
  const entry = exampleAudio[b.id];
  if (entry) playPrerendered(b.id, entry, playKanaBtn);
  else playKana(kanaEl.value, playKanaBtn);
});
...
// (?nopreload — 예제 음원 빌드 시엔 예열 인스턴스가 렌더와 메모리를 다퉈 v86 할당이 실패하므로 끈다)
